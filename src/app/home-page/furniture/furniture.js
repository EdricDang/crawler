 ...
 */
angular.module("app.home-page.furniture", []).config(function config($stateProvider) {
        $stateProvider.state('home-page.furniture', {
            url: '/furniture',
            views: {
                "home": {
                    controller: 'FurnitureCtrl',
                    templateUrl: 'home-page/furniture/furniture.tpl.html'
                }
            },
            data: {pageTitle: 'Furniture'}
        });
    })
    .controller('FurnitureCtrl', ['$log', '$scope', '$stateParams', '$state', 'toastr', 'security', 'homeServices',
        function FurnitureCtrl($log, $scope, $stateParams, $state, toastr, security, homeServices) {
            $log.info('FurnitureCtrl');
            //get list product
            $scope.list_product = {};
            ...
            homeServices.getIMGHightlight().success(function (data) {
                ...
                for (var h = 0; h < $scope.hgl.length; h++) {
                    if ($scope.hgl[h].key == 'funiture-page') {
